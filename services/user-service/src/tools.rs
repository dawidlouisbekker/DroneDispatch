...
//! Once implemented, shop and menu tools read from catalog-read-service, never from
...
payments run in test mode. Follow this protocol: \
1) find shops with search_nearby_shops (by category) or search_places (by name); only Drone Drop shops \
are returned, so read out their spoken names; \
2) show the menu with get_menu and call update_cart until the customer is happy; \
3) read back the shop, items, total, pickup location and ETA from the quote; \
4) call place_order only after the customer explicitly says yes. \
If place_order says the order is awaiting approval, tell the customer to approve it in the Drone Drop app. \
New pickup locations must be verified in the Drone Drop app before they can be used. \
You cannot add funds or change spending limits.";
...
pub struct SearchNearbyShops {
    ...
    pub category: String,
    /// Verified pickup location to search around. Defaults to the customer's default location.
...
pub struct SearchPlaces {
    /// Name of a specific shop, restaurant or place.
    pub query: String,
    /// Verified pickup location to bias results towards.
    pub pickup_location_id: Option<String>,
}

#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct PlaceId {
    /// The shop's `place_id` from a search result.
    pub place_id: String,
}
...
#[tool_router]
impl DroneDrop {
    #[tool(description = "Find Drone Drop shops of a category near a verified pickup location. Returns each shop's place_id and the spoken name to read out.")]
    ...
    #[tool(description = "Search for a specific shop by name. Returns only shops that are on Drone Drop, with their spoken names.")]
    fn search_places(&self, Parameters(params): Parameters<SearchPlaces>) -> String {
        not_implemented("search_places", &params)
    }

    #[tool(description = "Get a shop's address, whether it is accepting orders, prep time, distance and delivery ETA.")]
    fn get_shop_details(&self, Parameters(params): Parameters<PlaceId>) -> String {
        not_implemented("get_shop_details", &params)
    }

    #[tool(description = "Get a shop's menu items with spoken names, price, availability and weight.")]
    fn get_menu(&self, Parameters(params): Parameters<GetMenu>) -> String {
        not_implemented("get_menu", &params)
    }

    #[tool(description = "List shops the customer has ordered from and pickup locations they have used, with last order date and count.")]
    ...
    #[tool(description = "List the customer's previous orders, newest first.")]
    fn list_order_history(&self, Parameters(params): Parameters<OrderHistory>) -> String {
        not_implemented("list_order_history", &params)
    }

    #[tool(description = "Start a new cart with the items of a previous order. Read back the new quote before ordering.")]
    fn reorder(&self, Parameters(params): Parameters<OrderId>) -> String {
        not_implemented("reorder", &params)
    }

    #[tool(description = "List the customer's verified pickup locations and any still pending verification.")]
    fn list_pickup_locations(&self) -> String {
        not_implemented("list_pickup_locations", &())
    }

    #[tool(description = "Add a new pickup location. It stays pending until the customer verifies it with a passkey in the Drone Drop app; tell them to do so.")]
    fn request_new_pickup_location(&self, Parameters(params): Parameters<NewPickupLocation>) -> String {
        not_implemented("request_new_pickup_location", &params)
    }
    ...
    #[tool(description = "Place the order. Call only after reading back the quote and the customer explicitly saying yes. Orders above the customer's approval threshold wait for approval in the app; the business must accept before payment is captured.")]
    fn place_order(&self, Parameters(params): Parameters<PlaceOrder>) -> String {
        not_implemented("place_order", &params)
    }

    #[tool(description = "Get the live status of an order: approval, payment, business acceptance and drone progress.")]
